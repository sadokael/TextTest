...
{
    OBJECT *obj;
    for (obj = objs; obj < enfOfObjs; obj++)
    {
...
static int nounsIsInTags(const chat *noun, const char **tags)
{
    while (*tags != NULL)
    {
...
OBJECT *parseObject(const char *noun)
{
    ...
    {
        if (noun != NULL && nounsIsInTags(noun, obj->tags) && distanceTo(obj) < distanceTo(found))
        {
            found = obj;
        }
    }
...
int listObjectAtLocation(OBJECT *location)
{
    int count = 0;
    OBJECT *obj;
    for (obj = objs; obj < endOfObjs; obj++)
